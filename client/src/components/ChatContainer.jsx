...
const ChatContainer = () => {
  ...
  const handleSendImage = async(e)=>{
    ...
    // FileReader only starts reading the file when we call: 
  ...
  return selectedUser? (
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      ...
      {/* -------- bottom area--------*/}
      <div className="absolute bottom-0 left-0 righ-0 flex items-center gap-3 p-3">
        <div className="flex flex-1 items-center bg-gray-100/12 px-3 rounded-full">
          <input type="text"
            value={input}
            onChange={(e)=>setInput(e.target.value)}
            onKeyDown={(e)=>e.key==="Enter"?handleSendMessage(e):null}
           placeholder='Send a message' className="flex-1 text-sm 
          w-90 p-3  border-none rounded-lg outline-none text-white placeholder-gray-400"/>
